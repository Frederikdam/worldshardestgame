...
public class QTable {
    ...
    int getBestAction(char[] map){
        ...
        System.out.println(prevAction + " " + goalIndex);
        //if (avatarIndex == goalIndex) updateQvalue(1, map);

        float[] moves = getActionsQValues(map);
    ...
    int explore(char[] map){
        float[] availableActions = getValues(map);
    ...
    void updateQvalue(int reward, char[] map){
        // https://learnit.itu.dk/pluginfile.php/226942/mod_resource/content/1/Lab%205%20QLintro.pdf
        // slide 5-6

        String prevStateStr = String.valueOf(prevState);
        float[] qVal = table.get(prevStateStr);
        float prevActionQ = qVal[prevAction];

        int actionIndex = getBestAction(map);
        String stateStr = String.valueOf(map);
        float[] qValState = table.get(stateStr);
        float actionQ = qValState[actionIndex];

        qVal[prevAction] = (learningRate * (reward + (gammaValue*actionQ) - prevActionQ));
        table.put(prevStateStr, qVal);
    }
